...
var $title;              //title
var $subject;            //subject
var $author="Kim Howell Honor";             //author
...
function Output($name='', $dest='')
{
	...
	switch($dest)
	{
		...
		case 'D':
			...
			if(ob_get_length()){
                //echo ob_get_length();
				//$this->Error('Some data has already been output, can\'t send PDF file'); //remove for a while
            }
...
function _putinfo()
{
	$this->_out('/Producer '.$this->_textstring('NHTS Report Generator Class v'.FPDF_VERSION));
	if(!empty($this->title))
